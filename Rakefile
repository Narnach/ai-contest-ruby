...
task :debug do
  ENV['MAP'] = Dir.glob("maps/*.txt").rand if ENV['MAP']=='random'
  map = ENV['MAP'] || 'maps/map1.txt'

  ENV['BOT1'] = Dir.glob("bots/*.rb").map{|file| File.basename(file).gsub(".rb","")}.rand if ENV['BOT1']=="random"
  bot = ENV['BOT1'] || 'seer'

  ENV['BOT2'] = Dir.glob("bots/*.rb").map{|file| File.basename(file).gsub(".rb","")}.rand if ENV['BOT2']=="random"
  ...
  system %Q[java -jar tools/PlayGame.jar #{map} 1000 200 last_game.log "ruby MyBot.rb -v #{bot}" "ruby MyBot.rb -v #{bot2}"#{output}]
  exec "mate ./last_game.log"
end
...
task :tcp do
  ENV['BOT'] = Dir.glob("bots/*.rb").map{|file| File.basename(file).gsub(".rb","")}.rand if ENV['BOT']=="random"
  bot = ENV['BOT'] || ''
