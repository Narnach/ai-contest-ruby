...
* Send supply chains to the closest planet higher up the supply chain. Shorter lines means faster attacks.
...
* Double-check SniperStrategy target finder. It attacks planets that are not defensible. This can only work when sending a supply line straight after it. Otherwise, snipe closer targets.
* An example of attacks that go nowhere: http://ai-contest.com/visualizer.php?game_id=6222693
  * Sniperbot attacks the enemy's target with the intent of sniping it. After the initial fleet, there is no real follow-up. Fleets in the air towards hostile planets should get support. This should almost be a primary module. SupportSniperStrategy.
  * The RoI of sending 31 ships on an 11-turn journey is not very high. A 16-turn journey for 24 ships is even worse. In total, 78 ships are sent south in the first 10 turns, but there is no follow-up. It weakens the enemy, but it does not impact either player's growth. Actually, it does not prevent the enemy from capping, so ships are lost. The 78 population size-4 planet to the west would have been a better investment. It is 5 turns away and would have generated break-even in 19 turns. That is 8 turns after the failed attack on the 16-turn distance enemy planet. The 60-ship size-5 ship is a 12-turn recoup and should be a primary target.
  * I think the bot needs primary goals, as it currently starts to lose based on following low-level impulse-responses instead of a thought-through plan.
* Better protect planets under attack. CHeck the topmost small planet that keeps changing sides: http://ai-contest.com/visualizer.php?game_id=6303627
