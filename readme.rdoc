...
* Weakening the enemy when ahead will be a fallback scenario when my regeneration is ahead as well. But maybe go after their weakest planets instead of after their closest planets.
* Calculate the destination of all fleets, to calculate future regeneration and see if I should grow or attack.
* distance scoring of targets: d(me, planet) - d(enemy, planet). Negative is good, positive is bad.
* When capping, calculate defensibility of new location. Capping an enemy planet even for one turn gives an advantage in regen, but the opportunity cost may be to not cap a very defensible neutral with similar regen.
